...
class TerminalPane(Container):
    ...
    def on_resize(self, event: events.Resize) -> None:
        ...
        # Get the Terminal widget's size, not the Container's
        try:
            terminal_widget = self.query_one(Terminal)
            new_cols = terminal_widget.size.width
            new_rows = terminal_widget.size.height
            self.log(f"TerminalPane.on_resize(): new size {new_cols}x{new_rows}")
            
            if new_cols > 0 and new_rows > 0 and (new_cols != self.cols or new_rows != self.rows):
                self.cols = new_cols
                self.rows = new_rows
                # Update the Terminal widget's screen size
                terminal_widget.ncol = new_cols
                terminal_widget.nrow = new_rows
                # Resize the pyte screen - this preserves existing content
                terminal_widget._screen.resize(new_rows, new_cols)
                # Recreate the stream with the resized screen
                terminal_widget.stream = pyte.Stream(terminal_widget._screen)
                # Notify the PTY about the size change
                winsize = struct.pack("HHHH", new_rows, new_cols, 0, 0)
                fcntl.ioctl(self.fd, termios.TIOCSWINSZ, winsize)
                # Force a refresh to show the resized terminal
                terminal_widget.refresh()
        except Exception as e:
            self.log(f"on_resize error: {e}")
